/**
 * tari.js — pentru index.html (lista cu țări)
 * Funcții: Căutare țări, Favorite (accordion: Țări / Aeroporturi / Zboruri), Zboruri Urmărite
 */
...
(function () {
  ...
  ══════════════════════════════ */
  const TariStore = {
    getFavorites: () => JSON.parse(localStorage.getItem("flx_favorites") || "[]"),
    saveFavorites: (arr) => localStorage.setItem("flx_favorites", JSON.stringify(arr)),
    addFavorite(item) {
      const favs = this.getFavorites();
      if (!favs.find(f => f.id === item.id)) { favs.push(item); this.saveFavorites(favs); return true; }
      return false;
    },
  ...
  const renderTariSection = (inner, countEl) => {
    const favs = TariStore.getFavorites();
    countEl.textContent = favs.length;
    if (!favs.length) {
      inner.innerHTML = `<div class="flx-acc-empty">✈️<br>Nicio țară salvată.<br>Apasă ♡ lângă o țară.</div>`;
      return;
    }
    ...
    inner.querySelectorAll(".flx-fav-item").forEach(item => {
      item.onclick = e => {
        ...
        safeRedirect(item.dataset.href, "Pagina acestei țări nu a putut fi găsită.");
      };
    });
    inner.querySelectorAll(".flx-fav-remove").forEach(btn => {
      btn.onclick = e => {
        e.stopPropagation();
        TariStore.removeFavorite(btn.dataset.id);
  ...
  const renderAeroportSection = (inner, countEl) => {
    const favs = AeroportStore.get();
    countEl.textContent = favs.length;
    if (!favs.length) {
      inner.innerHTML = `<div class="flx-acc-empty">🛬<br>Niciun aeroport salvat.</div>`;
      return;
    }
    inner.innerHTML = favs.map(f => `
      <div class="flx-fav-item" data-href="${cleanHref(f.href)}">
  ...
  /* FIX: zboruri favorite în tari.js — afișează ruta "Din → Spre" */
  ...
  /* FIX: zboruri urmărite în tari.js — afișează ruta + redirect corect */
  ...
  let accSections = {};
  let favPanel, watchPanel, favBadge, watchBadge;

  const buildAccordionSection = (icon, title, key, panelId) => {
    const sec = document.createElement("div");
    sec.className = "flx-acc-section";
    sec.id = `flx-acc-${panelId}-${key}`;
    sec.innerHTML = `
      <div class="flx-acc-header">
        <div class="flx-acc-header-left">
          ...
          <span class="flx-acc-count" id="flx-acc-count-${panelId}-${key}">0</span>
        </div>
        <span class="flx-acc-arrow">▼</span>
      </div>
      <div class="flx-acc-body">
        <div class="flx-acc-inner" id="flx-acc-inner-${panelId}-${key}"></div>
      </div>
    `;
  ...
  const buildPanels = () => {
    ...
    [
      ...
      const sec = buildAccordionSection(icon, title, key, "fav");
    ...
    accSections.tari.section.classList.add("open");
  ...
  const updateBadge = () => {
    if (favBadge) {
      const n = TariStore.getFavorites().length + AeroportStore.get().length + ZboruriStore.get().length;
  ...
  const buildSearch = () => {
    ...
    wrapper.innerHTML = `
      <input id="flx-search" type="text" placeholder="Caută țară…" autocomplete="off">
      <div id="flx-no-results">Nicio țară găsită.</div>
    `;
    const anchor = document.querySelector(".layout") || lista;
    ...
    lista.querySelectorAll("li").forEach(li => {
      ...
      const href = hrefRaw && hrefRaw !== "#" ? new URL(hrefRaw, location.href).href : "";

      const star = document.createElement("button");
      star.className = "flx-fav-star";
      star.dataset.id = id;
      star.title = "Adaugă la favorite";
      star.textContent = TariStore.isFavorite(id) ? "❤️" : "♡";

      star.onclick = (e) => {
        e.preventDefault();
        e.stopPropagation();
        if (TariStore.isFavorite(id)) {
          TariStore.removeFavorite(id);
          star.textContent = "♡";
          toast("Eliminat din favorite", "info");
        } else {
          TariStore.addFavorite({ id, name, cod, href });
          star.textContent = "❤️";
...
const boot = () => {
  if (document.body.dataset.page !== "tari") return;
  if (!document.querySelector(".layout .lista")) return;
